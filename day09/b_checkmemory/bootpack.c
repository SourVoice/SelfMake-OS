...
{
    /*检查从start到end地址范围内能够使用的内存的末尾地址*/
    unsigned int i, *p, old;
    unsigned int pat0 = 0xaa55aa55, pat1 = 0x55aa55aa; /*向内存中试写入的值*/
    for (i = start; i < end; i += 0x1000)              /*i每次增加4KB*/
    {
        p = (unsigned int *)(i + 0xffc);
        old = *p;
        *p = pat0;
        *p ^= 0xffffffff; /*在内存中反转该值(pat0)*/
        if (*p != pat1)   /*检查反转结果(如果不正确)*/
        {
        not_memory:
            *p = old;
            break;
        }
        *p ^= 0xffffffff; /*如果正确则再次反转*/
        if (*p != pat0)   /*检查是否恢复*/
        {
            goto not_memory;
        }
